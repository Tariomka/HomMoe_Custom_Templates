...
func TestWhenDataIsSingleString_WrapsIntoSingleElementList(t *testing.T) {
	...
	value := gofakeit.Word()
	data := []byte(`"` + value + `"`)
	var list template_variant_entity.StringList
	...
	assert.Equal(t, template_variant_entity.StringList{value}, list)
}
...
func TestWhenDataIsArray_DecodesEveryString(t *testing.T) {
	...
	// Assert
	require.NoError(t, err)
	assert.Equal(t, template_variant_entity.StringList{"first", "second"}, list)
}

func TestWhenDataIsNull_SetsListToNil(t *testing.T) {
	t.Parallel()
	// Arrange
	list := template_variant_entity.StringList{gofakeit.Word()}
...
func TestWhenSingleStringIsUnterminated_ReturnsError(t *testing.T) {
	t.Parallel()
	// Arrange
	list := template_variant_entity.StringList{}
