...
func TestWhenAnglesDiffer_ReturnsExpectedShortestDistance(t *testing.T) {
	testCases := []struct {
		...
		{"WhenAnglesAreQuarterTurnApart_ReturnsHalfPi", 0, stdmath.Pi / 2, stdmath.Pi / 2},
		{"WhenAnglesAreHalfTurnApart_ReturnsPi", 0, stdmath.Pi, stdmath.Pi},
		{"WhenAnglesWrapAroundFullTurn_ReturnsShortWayDistance", 0.1, 2*stdmath.Pi - 0.1, 0.2},
		{"WhenRawDeltaExceedsPi_ReturnsComplementDistance", 0, 3 * stdmath.Pi / 2, stdmath.Pi / 2},
		{"WhenAnglesAreSeveralTurnsApart_WrapsIntoSingleTurn", 0, 4*stdmath.Pi + 0.3, 0.3},
...
func TestWhenArbitraryAnglesProvided_DistanceNeverExceedsPi(t *testing.T) {
	...
	// Assert
	assert.LessOrEqual(t, distance, stdmath.Pi)
}
