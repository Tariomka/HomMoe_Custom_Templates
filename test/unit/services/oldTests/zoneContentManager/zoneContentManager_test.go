...
func TestRowsToMandatoryContent_EmptyInputReturnsNil(t *testing.T) {
	...
	if out := contentProvider.CreateContentItemsFrom(nil); out != nil {
...
func TestRowsToMandatoryContent_BlankSidIsSkipped(t *testing.T) {
	...
	contentProvider := providers.NewMandatoryContentProvider()
	if out := contentProvider.CreateContentItemsFrom(rows); len(out) != 0 {
...
// 		},
// 	}}
// 	stripped := providers.StripNearCastleRules(items)
