...
func zone(name string) template.Zone { return template.Zone{Name: name} }
func zonePos(name string, x, y float64) template.Zone {
	z := zone(name)
	z.GeneratorPosition = pos(x, y)
	return z
}
func zoneRing(name string, ring int, x, y float64) template.Zone {
	z := zonePos(name, x, y)
	z.GeneratorRing = ringIdx(ring)
	return z
}
func conn(from, to string) template.Connection {
	return template.Connection{From: from, To: to, ConnectionType: "Direct"}
}

func tmpl(zones []template.Zone, conns []template.Connection) *template.RmgTemplateModel {
	return &template.RmgTemplateModel{
		Variants: []template.Variant{{Zones: zones, Connections: conns}},
	}
}
...
func TestBuildPreviewLayout_EmptyVariants(t *testing.T) {
	out := services.BuildPreviewLayout(&template.RmgTemplateModel{}, config.TopologyDefault, 600)
...
func TestBuildPreviewLayout_DefaultTopology_RingLayout(t *testing.T) {
	zones := []template.Zone{zone("Spawn-A"), zone("Spawn-B"), zone("Neutral-C")}
	conns := []template.Connection{conn("Spawn-A", "Spawn-B"), conn("Spawn-B", "Neutral-C")}
...
func TestBuildPreviewLayout_SingleZoneCentredOnCanvas(t *testing.T) {
	zones := []template.Zone{zone("Spawn-A")}
...
func TestBuildPreviewLayout_ExplicitHubGetsCentre(t *testing.T) {
	zones := []template.Zone{zone("Hub"), zone("Spawn-A"), zone("Spawn-B")}
	out := services.BuildPreviewLayout(tmpl(zones, nil), config.TopologyDefault, 600)
...
func TestBuildPreviewLayout_ImplicitHubGetsCentre(t *testing.T) {
	...
	zones := []template.Zone{zone("Spawn-A"), zone("Spawn-B"), zone("Neutral-H")}
	conns := []template.Connection{conn("Neutral-H", "Spawn-A"), conn("Neutral-H", "Spawn-B")}
	out := services.BuildPreviewLayout(tmpl(zones, conns), config.TopologyDefault, 600)
...
func TestBuildPreviewLayout_MultiHubFanLayout(t *testing.T) {
	zones := []template.Zone{zone("Hub-A"), zone("Hub-B"), zone("Spawn-A"), zone("Spawn-B")}
	conns := []template.Connection{
		conn("Hub-A", "Spawn-A"),
		conn("Hub-B", "Spawn-B"),
		conn("Hub-A", "Hub-B"),
	}
...
func TestBuildPreviewLayout_RandomTopology_WithPositions(t *testing.T) {
	...
	conns := []template.Connection{conn("Spawn-A", "Neutral-C"), conn("Neutral-C", "Spawn-B")}
...
func TestBuildPreviewLayout_BalancedTopology_SingleZone(t *testing.T) {
	zones := []template.Zone{zoneRing("Spawn-A", 0, 0.5, 0.5)}
	out := services.BuildPreviewLayout(tmpl(zones, nil), config.TopologyBalanced, 600)
...
func TestBuildPreviewLayout_DirectConnectionsAreCollected(t *testing.T) {
	zones := []template.Zone{zone("Spawn-A"), zone("Spawn-B")}
	conns := []template.Connection{conn("Spawn-A", "Spawn-B")}
	out := services.BuildPreviewLayout(tmpl(zones, conns), config.TopologyDefault, 600)
...
func TestBuildPreviewLayout_ConnectionWithUnknownZoneSkipped(t *testing.T) {
	zones := []template.Zone{zone("Spawn-A"), zone("Spawn-B")}
	conns := []template.Connection{conn("Spawn-A", "Missing-X")}
	out := services.BuildPreviewLayout(tmpl(zones, conns), config.TopologyDefault, 600)
...
func TestBuildPreviewLayout_CityMainObjectCounts(t *testing.T) {
	zones := []template.Zone{
		{Name: "Neutral-Z", MainObjects: []template.MainObject{{Type: "City"}, {Type: "City"}}},
	}
	out := services.BuildPreviewLayout(tmpl(zones, nil), config.TopologyDefault, 600)
...
func TestClassifyZoneTier_SpawnZoneIsZero(t *testing.T) {
	if got := services.ClassifyZoneTier(template.Zone{Name: "Spawn-A"}); got != 0 {
		t.Errorf("got %d, want 0", got)
	}
}

func TestClassifyZoneTier_GuardedPoolT5IsGold(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", GuardedContentPool: []string{"classic_template_pool_random_t5_item"}}
...
func TestClassifyZoneTier_GuardedPoolT4IsGold(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", GuardedContentPool: []string{"classic_template_pool_random_t4_item"}}
	if got := services.ClassifyZoneTier(z); got != 3 {
		t.Errorf("got %d, want 3", got)
	}
}
...
func TestClassifyZoneTier_GuardedPoolT3IsSilver(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", GuardedContentPool: []string{"classic_template_pool_random_t3_item"}}
...
func TestClassifyZoneTier_GuardedPoolT2IsBronze(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", GuardedContentPool: []string{"classic_template_pool_random_t2_item"}}
...
func TestClassifyZoneTier_UnguardedPoolFallback(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", UnguardedContentPool: []string{"classic_template_pool_random_unguarded_t3_item"}}
	if got := services.ClassifyZoneTier(z); got != 2 {
		t.Errorf("got %d, want 2", got)
	}
}
...
func TestClassifyZoneTier_LayoutSidesIsBronze(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", Layout: "zone_layout_sides"}
	if got := services.ClassifyZoneTier(z); got != 1 {
		t.Errorf("got %d, want 1", got)
	}
}
...
func TestClassifyZoneTier_LayoutTreasureIsSilver(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", Layout: "zone_layout_treasure_zone"}
	if got := services.ClassifyZoneTier(z); got != 2 {
		t.Errorf("got %d, want 2", got)
	}
}
...
func TestClassifyZoneTier_LayoutCenterIsGold(t *testing.T) {
	z := template.Zone{Name: "Neutral-A", Layout: "zone_layout_center"}
	if got := services.ClassifyZoneTier(z); got != 3 {
		t.Errorf("got %d, want 3", got)
	}
}
...
func TestClassifyZoneTier_NameLowFallback(t *testing.T) {
	z := template.Zone{Name: "Neutral-low"}
	if got := services.ClassifyZoneTier(z); got != 1 {
		t.Errorf("got %d, want 1", got)
	}
}
...
func TestClassifyZoneTier_NameMedFallback(t *testing.T) {
	z := template.Zone{Name: "Neutral-med"}
	if got := services.ClassifyZoneTier(z); got != 2 {
		t.Errorf("got %d, want 2", got)
	}
}
...
func TestClassifyZoneTier_NameHighFallback(t *testing.T) {
	z := template.Zone{Name: "Neutral-high"}
	if got := services.ClassifyZoneTier(z); got != 3 {
		t.Errorf("got %d, want 3", got)
	}
}
...
func TestClassifyZoneTier_NameCoreFallback(t *testing.T) {
	z := template.Zone{Name: "Neutral-core"}
	if got := services.ClassifyZoneTier(z); got != 3 {
		t.Errorf("got %d, want 3", got)
	}
}
...
func TestClassifyZoneTier_UnknownDefaultsToBronze(t *testing.T) {
	z := template.Zone{Name: "Neutral-Z"}
	if got := services.ClassifyZoneTier(z); got != 1 {
		t.Errorf("got %d, want 1", got)
	}
}
