...
func NewWarningBannerWidget(theme *material.Theme, message string) layout.Widget {
	return func(gtx layout.Context) layout.Dimensions {
		...
		dims := layout.UniformInset(constants.DefaultPadding).
			Layout(gtx, NewLabelWidget(theme, message, themes.ColorsBase.WarnText))
		call := macro.Stop()
		radius := gtx.Dp(constants.DefaultRoundness)
		rect := image.Rectangle{Max: dims.Size}
		paint.FillShape(gtx.Ops, themes.ColorsBase.WarnBackground, clip.UniformRRect(rect, radius).Op(gtx.Ops))
		paint.FillShape(gtx.Ops, themes.ColorsBase.WarnBorder, clip.Stroke{
