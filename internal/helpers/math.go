...
func CalculatePointTowards(from, toward data.Vec2[float64], distance float64) (movedPoint data.Vec2[float64], ok bool) {
	delta := toward.Subtract(from)
	deltaDistance := math.Hypot(delta.X, delta.Y)
	if deltaDistance < 1 {
		return movedPoint, false
	}

	return from.Add(delta.DivideScalar(deltaDistance).MultiplyScalar(distance)), true
}
