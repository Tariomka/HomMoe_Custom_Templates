...
type ManageRulesDialog struct {
	...
	scroll           widget.List
	typeDropdown     *DropdownSelector
	distanceDropdown *DropdownSelector
	variantDropdown  *DropdownSelector
...
func NewManageRulesDialog(mapping models.SidMapping, rules []models.ContentRuleRowSave, onApply func([]models.ContentRuleRowSave)) *ManageRulesDialog {
	...
	dialog.typeDropdown = NewDropdownSelector(typeLabels)
	dialog.distanceDropdown = NewDropdownSelector(content_rules.GetDistanceDisplayNames())
	dialog.variantDropdown = NewDropdownSelector(dialog.variantLabels)
...
func (this *ManageRulesDialog) Body(gtx layout.Context, theme *material.Theme) (layout.Dimensions, bool) {
	if this.applyBtn.Clicked(gtx) {
		if this.onApply != nil {
			this.onApply(cloneRuleRows(this.rules))
		}
