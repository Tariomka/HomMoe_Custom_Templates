...
- Owner-review follow-up (2026-09-10): owner committed Batch B as `252a7ee`, excluding the untracked PNG capture utility. At the owner's request, removed that environment-gated utility and made fixture enumeration private; unconditional pixel regression tests and all their fixtures remain. Future GUI flow/export golden verification remains separate work. No production changes. Cleanup build, default tests, fresh unit coverage and layout pass; coverage 74.5% before/after, final lint zero. No Git index changes.
...
- [x] Keep unit tests pixel-only and in-memory. Capture representative pre-fix PNGs for owner review in an ignored diagnostic directory. The temporary capture utility was subsequently removed at owner request; it was not a regression assertion or an application export path. Existing review images are historical evidence, not golden fixtures.
...
Two production lines changed. Focused regressions pass. Eighteen before/after pairs are under `output/research/batch-b-png/20260910-101656`; four later fixtures are after-only and were disclosed. Owner selected **Approved** for the linked short portal, longer portal and short direct-edge PNG comparisons on 2026-09-10. No schema, default, output authorization or Wire changes. Fixed layouts remain in a test helper for unconditional pixel assertions; review-only artifact capture was removed at owner request. The zero-length and failed-trimming branches both have public-API coverage.
