...
4c** (make Revert to Base a preview that only commits on Apply).
...
| [todo/review-opus5-08-04.md](../todo/review-opus5-08-04.md) | §0.2 row rewritten for the new design. |
| [plans/zone-editor-state-extraction.md](../plans/zone-editor-state-extraction.md) | Phase 4 and 4b marked superseded; **Phase 4c** added and completed. |
...
Branch **`AD/refactoring-07-21`**. The owner has staged some Phase 4 files
(`M ` / `MM` first column) — per AGENTS.md §2.5 they were **not** unstaged.
Nothing was committed by the agent.

```
M  .agent/session-carry-forward.md
MM app/gui/dialogs/zoneEditorDialog.go
M  app/gui/dialogs/zoneEditorDialog_testexports.go
 M app/gui/drivers/state.go
MM app/gui/drivers/stateGeneration.go
MM app/gui/drivers/stateManualEdits.go
MM app/gui/panels/layoutPanelZones.go
AM internal/dtos/zoneEditorZonesDto.go
MM plans/zone-editor-state-extraction.md
M  test/integration/editorState_integration_test.go
MM test/integration/gui/zoneEditorDialog_integration_test.go
M  test/integration/gui/zoneEditorGeometry_integration_test.go
M  test/integration/manualCastleReapply_integration_test.go
AM test/integration/zoneEditorRevertToBase_integration_test.go
M  test/unit/app/gui/drivers/state/getTemplateRevision_test.go
MM test/unit/app/gui/drivers/stateManualEdits/applyEditedZones_test.go
AD test/unit/app/gui/drivers/stateManualEdits/revertZonesToBase_test.go
MM todo/review-opus5-08-04.md
?? test/unit/app/gui/drivers/stateManualEdits/previewBaseZones_test.go
```
...
1. Owner reviews and runs the build to confirm that Revert to Base now only
   previews, that Cancel keeps the edited layout, and that Apply commits it.
2. **Phase 5 — Close out** of
   [plans/zone-editor-state-extraction.md](../plans/zone-editor-state-extraction.md):
   - `wire diff ./internal/composition/...` (exit 0 = up to date; `wire gen`
     writes its banner to stderr, so never trust its exit code).
   - Performance benchmarks — quote `"-bench=."`, PowerShell mangles it bare.
   - `$env:GOOS='linux'` lint pass, then `Remove-Item Env:\GOOS`.
   ...
   - Update [todo/test_observations.md](../todo/test_observations.md): it still
     lists `zoneEditorDialog.go` and its canvas/snap/property files as
     untestable Gio territory, which is no longer true.
3. Stop for owner review. **Do not stage. Do not commit.**
...
> by `gofmt -l`.
>
> Where work left off: Batch 15 Phase 4c is **complete and green** — the manual
> zone editor's single "Revert changes" button was replaced by **"Undo"**
> (one-shot, in-session) and **"Revert to Base"**, which calls
...
> resurrect them. Build, vet (both tag combinations), testlayoutcheck, unit,
> integration and GPU-gated GUI suites all pass; `gofmt -l` is empty;
> `golangci-lint-v2` reports 0 issues; coverage is 72.5%.
>
> Next up is **Phase 5 — Close out** of
> `plans/zone-editor-state-extraction.md`. Nothing is staged by the agent, and
> the owner's own staged files must be left alone.
>
