...
3. **Verify every claim against the source.** No finding may be based on memory,
   ...
   `.agent/backlog`), and re-read the file immediately before writing the finding.
...
4. **Audit prior reviews first.** If earlier review documents or observation
   files exist (e.g. `.agent/backlog/review*.md`, `.agent/backlog/test_observations.md`,
   `.agent/backlog/backlog.md`), give a disposition for **every** prior item: Fixed (with
